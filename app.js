 ...
 * Static SPA powered by hash-based routing
 * Reads blogs/index.json + photos/index.json auto-generated by GitHub Actions
 */
...
async function route() {
  ...
  document.querySelectorAll('.nav-link, .mob-link').forEach(el => {
    el.classList.toggle('active', el.dataset.page === page || (page === '' && el.dataset.page === 'home'));
  });

  app.innerHTML = '<div class="loading"><div class="loading-dot"></div><div class="loading-dot"></div><div class="loading-dot"></div></div>';
  ...
  try {
    ...
    console.error(e);
    app.innerHTML = `<div class="empty-state"><p>Something went wrong loading this page.</p></div>`;
  }
...
async function getBlogSlugs() {
  try {
    return await fetchJSON('blogs/index.json');
  } catch { return []; }
}

async function getPhotoFiles() {
  try {
    return await fetchJSON('photos/index.json');
  } catch { return []; }
}

/**
 * Parse a blog txt file.
 * Format:
 *   Title: Some Title
 *   Date: 2024-03-15
 *   Type: poetry | blog
...
async function renderHome() {
  ...
  const recent = slugs.slice(-3).reverse();
  const posts = [];
  for (const slug of recent) {
    try {
      const raw = await fetchText(`blogs/${slug}.txt`);
      posts.push(parseBlogTxt(raw, slug));
    } catch { /* skip broken */ }
  }

  const postCardsHTML = posts.length === 0
    ? `<div class="empty-state">
        <div class="empty-state-icon">✦</div>
        <p>No writing yet — add <code>.txt</code> files to the <code>blogs/</code> folder.</p>
       </div>`
    ...
    <div>
      ...
      <section class="hero">
        ...
        <p class="hero-tagline">
          Between the silence of circuits and the noise of solitude,<br/>
          I write.
        </p>
        <p class="hero-desc">
    ...
    <div>
      ...
      <div class="posts-list">
        <div class="posts-grid">
          ${posts.map(p => postCard(p, photos)).join('')}
        </div>
      </div>
    </div>
  `;
...
async function renderPost(slug) {
  ...
  let raw;
  try {
    raw = await fetchText(`blogs/${slug}.txt`);
  } catch {
  ...
  const photoFile = photoExists(photos, slug);

  const coverHTML = photoFile
    ? `<img class="post-cover-full" src="photos/${photoFile}" alt="${post.title}" />`
    : '';
  ...
  const contentHTML = isPoetry
    ...
    : `<div class="post-content">${post.content.split(/\n{2,}/).map(p => `<p>${p.replace(/\n/g, '<br>')}</p>`).join('')}</div>`;
...
async function renderGallery() {
  ...
  // Photos that do NOT have a matching blog post
  ...
  const gridHTML = galleryPhotos.length === 0
    ? `<div class="empty-state">
        ...
        <p>Photos named differently from blog files appear here.<br/>
    ...
    <div class="about-page">
      ...
      <div class="about-body">
        <div class="about-text">
          <p>
            I am an electrical engineer by training — someone who understands 
            how the world moves through wires, how current finds its path,
            how silence hums at the right frequency.
          </p>
          <p>
            I write because the equations never quite explain 
            <em>why the room feels empty</em> at 2 a.m., or why a photograph 
            of a power line against a grey sky can make you feel 
            impossibly small and strangely alive.
          </p>
          <p>
            This is my corner of the internet — for poetry, for reflections,
            for photographs taken during long walks through a world 
            that I am still trying to understand.
          </p>
          <p>
            <em>Unbearably lonely. Endlessly curious.</em>
          </p>
        </div>
...
function postCard(post, photos) {
  const photoFile = photoExists(photos, post.slug);
  const coverHTML = photoFile
    ? `<img class="post-card-cover" src="photos/${photoFile}" alt="${post.title}" loading="lazy" />`
